...
vector<T> rankedMobius(Function<T> &f,int n,int subsetRank) //only difference to fastMobius is the initalization
{
    ...
    for(int k=0; k<pow(2,n); k++)
    {
        if(__builtin_popcount (k)==subsetRank)  //__builtin_popcount counts bits set to 1, on hardware level if possible
        {
            d[0][k]=f(k);
        }
        else
...
vector<T> advanced_convolute(Function<T> &f,int n)
{
    ...
    for(int i=0; i<=n; i++)
    {
        ...
        fast_ranked_transform[i]=rankedMobius(f,n,i); //2 mal das gleiche, da dies  eig für 2 funktionen getan werden muss
    }
