...
static void mobius_join_node(int8_t *child1, int8_t *child2,int minc_1,int minc_2, int node_count,int8_t* result) {
    ...
    for (set_t dom_set = 0; dom_set < (1 << node_count); ++dom_set) {
        ...
        for(int set_idx=0;set_idx<set_count;++set_idx){
            int not_dom_index=calculate_not_dom_index(dom_set,not_dominated_sets[set_idx],node_count);
            ...
            ++set_counter;
            not_dominated_sets[set_idx]=index; //replace set by set index, the bit representation is not needed anymore
        }
                ...
                for(int temp_result_idx=0;temp_result_idx<set_count;++temp_result_idx){
                    if(temp[temp_result_idx]>0){
                        if(result[not_dominated_sets[temp_result_idx]]>i+j){
                            result[not_dominated_sets[temp_result_idx]]=i+j; //Todo maybe -#1?
                        }
                    }
                }
            }
